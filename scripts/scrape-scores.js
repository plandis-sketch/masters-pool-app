...
async function scrapeAndUpdate() {
  ...
  // NOW — using the existing Firestore scores — before this run overwrites them
  ...
  if (prevRound >= 1 && prevRound <= 3 && espnRound > prevRound) {
    const transRef = doc(db, 'tournaments', tournament.id, 'dailyLeaderboards', 'round' + prevRound);
    const transExisting = await getDoc(transRef);
    if (!transExisting.exists()) {
      ...
      console.log(`Saved Daily Leaderboard snapshot for Round ${prevRound} (round transition, pre-update)`);
    }
  }
